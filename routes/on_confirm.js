...
import { handleOnConfirmRequest } from "../services/on_confirm_service.js";
...
function createNackResponse(context, errorDetails) {
    console.warn(`[BAP Route] Sending NACK for /on_confirm: Type=${errorDetails?.type}, Code=${errorDetails?.code}, Msg=${errorDetails?.message}`);
    return {
        message: { ack: { status: "NACK" } },
        error: {
            type: errorDetails?.type || "CORE-ERROR",
            code: errorDetails?.code || "50000",
            message: errorDetails?.message || "BAP failed to process the on_confirm request"
        }
    };
}
